...
- 🪶 **活跃群友几乎不占写入额度**：插件要看每条群消息来维护活跃名单，但写入次数不跟着消息数涨——只记到「天」（同一个人一天最多记一次）、一个群一行、当天新冒出来的人攒一会儿再一起写（同一个群最多每 5 分钟写 1 行），不活跃的群友在写入时顺手剔掉。一个群每天的写入不超过当天活跃的人数，通常只有十几行。
...
- `only_record_at_message`：只把 @机器人的群友算作活跃成员（默认关闭）

0.3.0 起活跃名单改为每个群一行（新表 `members`），去掉了 `active_user_throttle_minutes`（旧配置里留着也不影响）。升级前一人一行的 `active_users` 不再写入，第一次用到某个群时自动并进新表，旧表保留。
